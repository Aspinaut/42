...
void ft_putnbr_hex(t_arg *arg, unsigned int nb, unsigned int converter)
{
	...
	if (nb > 15)
	{
		...
		if (remainder < 10)
		{
			ft_putnbr_hex(arg, nb, converter);
		...
		arg->len += ft_putchar(nb + (converter - 23) - 10);
	else
		arg->len += ft_putchar(nb + '0');
}
